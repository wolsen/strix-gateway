...
|-----|---------------------|---------|-------------|
| `iscsi-portal-ip` | `STRIX_ISCSI_PORTAL_IP` | `0.0.0.0` | iSCSI portal listen address |
...
| Key | Environment Variable | Default | Description |
|-----|---------------------|---------|-------------|
| `vhost-enabled` | `STRIX_VHOST_ENABLED` | `false` | Enable TLS vhost multiplexing |
| `vhost-domain` | `STRIX_VHOST_DOMAIN` | *(empty)* | Domain suffix for subsystem FQDNs (required if vhost enabled) |
| `vhost-hostname-override` | `STRIX_VHOST_HOSTNAME_OVERRIDE` | *(auto-detect)* | Override the hostname component of FQDNs |
| `vhost-require-match` | `STRIX_VHOST_REQUIRE_MATCH` | `true` | Return 404 for unrecognised Host headers |

### TLS Settings

| Key | Environment Variable | Default | Description |
|-----|---------------------|---------|-------------|
| `tls-mode` | `STRIX_TLS_MODE` | `per-subsystem` | Certificate mode: `per-subsystem` or `wildcard` |
| `tls-rotate-before-days` | `STRIX_TLS_ROTATE_BEFORE_DAYS` | `30` | Reissue certs expiring within this many days |

### Bind Settings

| Key | Environment Variable | Default | Description |
|-----|---------------------|---------|-------------|
| `bind-https-port` | `STRIX_BIND_HTTPS_PORT` | `443` | HTTPS listen port (when vhost enabled) |
| `bind-http-port` | `STRIX_BIND_HTTP_PORT` | `0` | HTTP listen port (0 = disabled) |
...
# Change HTTPS port
sudo snap set strix-gateway bind-https-port=8443

# Use wildcard certificate mode
sudo snap set strix-gateway tls-mode=wildcard

# Override hostname detection
sudo snap set strix-gateway vhost-hostname-override=gw01

# View current settings
sudo snap get strix-gateway
```
...
```
/var/snap/strix-gateway/common/tls/
...
```bash
sudo cp /var/snap/strix-gateway/common/tls/ca.{key,crt} /safe/backup/
```
