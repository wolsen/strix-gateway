...
async def test_reconcile_with_pool_and_volume(session_factory, spdk, cfg):
...
async def test_reconcile_volume_with_unknown_pool_is_skipped(session_factory, spdk, cfg):
    """A volume referencing a non-existent pool should be skipped gracefully."""
    from apollo_gateway.core.db import Volume

    async with session_factory() as session:
        vol = Volume(name="orphan", pool_id="nonexistent-pool", size_mb=512, status="available")
        session.add(vol)
        await session.commit()
...
async def test_reconcile_with_export_containers_and_mappings(session_factory, spdk, cfg):
...
async def test_reconcile_ensure_pool_failure_is_non_fatal(session_factory, spdk, cfg):
...
async def test_reconcile_ensure_lvol_failure_is_non_fatal(session_factory, spdk, cfg):
    ...
    async with session_factory() as session:
        ...
        session.add(pool)
        await session.flush()
        session.add(Volume(name="lf-vol", pool_id=pool.id, size_mb=256, status="available"))
        await session.commit()
...
async def test_reconcile_ensure_export_failure_is_non_fatal(session_factory, spdk, cfg):
...
async def test_reconcile_mapping_with_missing_volume_or_ec_is_skipped(session_factory, spdk, cfg):
...
async def test_reconcile_ensure_mapping_failure_is_non_fatal(session_factory, spdk, cfg):
