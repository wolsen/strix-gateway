...
async def reconcile(
    ...
    async with session_factory() as session:
        ...
        for pool in pools.values():
            try:
                await asyncio.to_thread(ensure_pool, spdk_client, pool)
            except Exception as exc:
                logger.error("Failed to reconcile pool %s: %s", pool.id, exc)

        # 4. Volumes (skip those in terminal error state)
        ...
        for vol in volumes.values():
            ...
            if pool is None:
                ...
                continue
            try:
                bdev_name = await asyncio.to_thread(ensure_lvol, spdk_client, vol, pool.name)
