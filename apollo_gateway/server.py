...
from apollo_gateway.core.db import Array, get_session_factory, init_db
from apollo_gateway.tls.manager import TLSManager
from apollo_gateway.tls.sni import SNIRouter
from apollo_gateway.tls.vhost import resolve_array_fqdn

logger = logging.getLogger("apollo_gateway.server")
...
def main() -> None:
    ...
    if not settings.vhost_domain:
        raise SystemExit(
            "APOLLO_VHOST_DOMAIN must be set when vhost mode is enabled"
        )
    ...
    import apollo_gateway.server as _self
...
# Allow ``python -m apollo_gateway.server``
